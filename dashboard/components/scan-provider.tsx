...
export function ScanProvider({ children }: { children: React.ReactNode }) {
  ...
  async function runScan(request: ScanRequest) {
    ...
    try {
      ...
      setReport(next)
      setHistory((items) => [{ scan_id: response.scan_id, url: request.url, page_title: request.page_title, scan_timestamp: request.scan_timestamp, overall_score: response.overall_score, risk_level: response.risk_level, total_fields: request.fields.length, critical_fields: response.field_results.filter((field) => field.sensitivity.toLowerCase() === "critical").length }, ...items])
      setStatus("success")
      return true
    } catch (cause) {
      setStatus("error")
  ...
  const value = useMemo(() => ({ report, history, status, error, runScan, resetError: () => { setError(null); setStatus("idle") } }), [report, history, status, error])
